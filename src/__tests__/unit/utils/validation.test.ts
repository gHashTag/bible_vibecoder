...
describe('Validation Utils', () => {
  ...
  describe('isValidInstagramUrl', () => {
    ...
    it('should return false for non-Instagram URLs', () => {
      ...
      expect(isValidInstagramUrl('https://insta.gram.com/username')).toBe(
        false
      );
    });
    ...
    it('should return true for valid Instagram URLs', () => {
      ...
      expect(isValidInstagramUrl('https://www.instagram.com/username')).toBe(
        true
      );
      expect(isValidInstagramUrl('http://www.instagram.com/username')).toBe(
        true
      );
    });
  });
  ...
  describe('extractUsernameFromUrl', () => {
    it('should return null for invalid Instagram URLs', () => {
      ...
      expect(
        extractUsernameFromUrl('https://facebook.com/username')
      ).toBeNull();
    });

    it('should return username from valid Instagram profile URLs', () => {
      expect(extractUsernameFromUrl('https://instagram.com/username')).toBe(
        'username'
      );
      expect(
        extractUsernameFromUrl('https://www.instagram.com/username/')
      ).toBe('username');
      expect(
        extractUsernameFromUrl('https://instagram.com/username?hl=en')
      ).toBe('username');
    });

    it('should return null for Instagram URLs that are not profile URLs', () => {
      expect(
        extractUsernameFromUrl('https://instagram.com/p/12345')
      ).toBeNull();
      expect(
        extractUsernameFromUrl('https://instagram.com/reel/12345')
      ).toBeNull();
      expect(
        extractUsernameFromUrl('https://instagram.com/reels/12345')
      ).toBeNull();
      expect(
        extractUsernameFromUrl('https://instagram.com/stories/12345')
      ).toBeNull();
      expect(
        extractUsernameFromUrl('https://instagram.com/explore')
      ).toBeNull();
      expect(
        extractUsernameFromUrl('https://instagram.com/accounts/login')
      ).toBeNull();
      expect(
        extractUsernameFromUrl('https://instagram.com/tags/travel')
      ).toBeNull();
    });
